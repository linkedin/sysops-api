...
import seco.range.sync.glu
import seco.range.sync.members
import seco.range.sync.index
import seco.range.sync.version
import seco.range.sync.access_groups
...
def range_data_merge(main_data, added_data):
  ...
  for cluster in main_data:
    if cluster in added_data:
      if not added_data[cluster]:
        log.warn("Missing cluster for: {0}".format(cluster))
      else:
        # TODO uncomment this once we turn down logging (from debug -> warn?)
        # Current prod logging is WAY too chatty to enable this now.
        #log.debug("Updating cluster: {0}".format(cluster))
...
def outputter(clusters, outdir, output_type='yaml', clean=True, protected=[]):
  ...
  if output_type != 'yaml':
    ...
    try:
      ...
      log.error("Type error (no CLUSTER tag?) for '{0}': {1}".format(cluster,e))
      raise TypeError("Missing CLUSTER key (?) for: '{0}'".format(cluster))
    finally:
