...
def validate_json_data(host_stats, site, local_disk, workdir):
  ...
  if local_disk:
    ...
    # Length is 5 because we initialize host_stats at the start of generate_json_data.
    if len(host_stats) == 5:
      try:
  ...
  for datastructure in ['total_redis_interaction_time', 'total_redis_transfer_speed']:
    for calc in ['mean', 'median', 'var']:
      host_stats[datastructure][calc] = map(float, host_stats[datastructure][calc])
  ...
  for datastructure in ['per_redis_server_transfer_speed', 'per_redis_server_interaction_time']:
    for redis_server in host_stats[datastructure]:
      host_stats[datastructure][redis_server]['median'] = map(float, host_stats[datastructure][redis_server]['median'])
      host_stats[datastructure][redis_server]['mean'] = map(float, host_stats[datastructure][redis_server]['mean'])
...
def generate_json_data(sites, workdir, refresh):
  ...
  for site in sites:
    ...
    # Delete the data and rebuild if its older than 60 minutes or if --refresh was passed on the CLI
    ...
    if not os.path.exists(workdir + '/' + site + '-host_stats.json'):
      ...
      # The datacenter is complete.  Dump all data and move onto the next datacenter in daemon mode.
      try:
        # Write to a temporary file so data isn't inserted into sysops-api until the write has completed.
      ...
      if data.get('global_total'):
        if data['global_total'].get(datastructure): 
          if data['global_total'][datastructure].get('median'):
            ...
            host_stats['machines'][host][datastructure]['median'] = data['global_total'][datastructure]['median']
            host_stats['machines'][host][datastructure]['mean'] = data['global_total'][datastructure]['mean']
            host_stats['machines'][host][datastructure]['var'] = data['global_total'][datastructure]['var']
            host_stats[datastructure]['median'].append(host_stats['machines'][host][datastructure]['median'])
            host_stats[datastructure]['mean'].append(host_stats['machines'][host][datastructure]['mean'])
            host_stats[datastructure]['var'].append(host_stats['machines'][host][datastructure]['var'])
        ...
        if data['per_mps'].get(datastructure):
          ...
          for redis_server in data['per_mps'][datastructure].iterkeys():
            ...
            if not host_stats[datastructure].get(redis_server):
              ...
              host_stats[datastructure][redis_server]['median'] = []
              host_stats[datastructure][redis_server]['mean'] = []
              host_stats[datastructure][redis_server]['var'] = []

            if data['per_mps'][datastructure][redis_server].get('median'):
              host_stats['machines'][host][datastructure][redis_server]['median'] = data['per_mps'][datastructure][redis_server]['median']
              host_stats['machines'][host][datastructure][redis_server]['mean'] = data['per_mps'][datastructure][redis_server]['mean']
              host_stats['machines'][host][datastructure][redis_server]['var'] = data['per_mps'][datastructure][redis_server]['var']
              host_stats[datastructure][redis_server]['median'].append(host_stats['machines'][host][datastructure][redis_server]['median'])
              host_stats[datastructure][redis_server]['mean'].append(host_stats['machines'][host][datastructure][redis_server]['mean'])
              host_stats[datastructure][redis_server]['var'].append(host_stats['machines'][host][datastructure][redis_server]['var'])
  ...
  for key in cache_results._gold.iterkeys():
    ...
    for line in cache_results._gold[key].splitlines():
      if "Currently Active Slave" in line:
        host_stats['machines'][host]['active_interface']['name'] = line.split(':')[1].strip()
...
def find_lldpd_data(datacenter, host_stats):
      ...
      try:
        ...
        for interface in interfaces:
          if interface.attributes['name'].value == host_stats['machines'][host]['active_interface']['name']:
            ...
            host_stats['machines'][host]['active_interface']['switch_name'] = switch_name.firstChild.toxml()
            port = interface.getElementsByTagName('port')[0]
            port_descr = port.getElementsByTagName('descr')[0]
            host_stats['machines'][host]['active_interface']['network_port'] = port_descr.firstChild.toxml()
      except Exception, e:
        pass
  ...
  for host in bad_hosts: 
    if not host_stats['machines'][host]['active_interface'].get('switch_name'):
      host_stats['machines'][host]['active_interface']['switch_name'] = "N/A"
      host_stats['machines'][host]['active_interface']['network_port'] = "N/A"
...
def pull_netstat_summary(datacenter, host_stats):
  ...
  for key in cache_results._gold.iterkeys():
    ...
    if not host_stats['machines'].get(host) or ".nw.linkedin.com" in host:
      continue
    host_stats['machines'][host]['active_interface']['netstat_summary'] = {}
    for line in cache_results._gold[key].splitlines():
      ...
      for word in words:
        if is_number(word):
          ...
          keyname.append(word)
      if value:
        host_stats['machines'][host]['active_interface']['netstat_summary']["_".join(keyname)] = value

    # Now that we have extracted all netstat summary data, start to build some statistics that aren't available out of the box.
	...
	host_stats['machines'][host]['active_interface']['netstat_summary'].get('tcp_segments_send_out')):
      host_stats['machines'][host]['active_interface']['netstat_summary']['tcp_segments_retransmited_ratio'] = float(float(host_stats['machines'][host]['active_interface']['netstat_summary']['tcp_segments_retransmited']) / float(host_stats['machines'][host]['active_interface']['netstat_summary']['tcp_segments_send_out']))
    ...
    if (host_stats['machines'][host]['active_interface']['netstat_summary'].get('udp_packet_receive_errors') and
      ...
      host_stats['machines'][host]['active_interface']['netstat_summary']['udp_packet_recieve_errors_ratio'] = float(float(host_stats['machines'][host]['active_interface']['netstat_summary']['udp_packet_receive_errors']) / float(host_stats['machines'][host]['active_interface']['netstat_summary']['udp_packets_received']))
...
def print_basic_host_info(host, host_stats, stat, sort, per_mps):
  ...
  if stat == "mean" or stat == "median" or stat == "var":
    host_data += stat + " bytes/s:" + str(int(host_stats['machines'][host]['total_redis_transfer_speed'][stat])).ljust(13)
    host_data += stat + " seconds:" + str(float('%.3f' % host_stats['machines'][host]['total_redis_interaction_time'][stat])).ljust(10)
  else:
    host_data += stat + ":" + host_stats['machines'][host]['active_interface']['netstat_summary'][stat].ljust(20)
  host_data += "interface: " + host_stats['machines'][host]['active_interface']['name'].ljust(10)
  host_data += "switch: " + host_stats['machines'][host]['active_interface']['switch_name'].ljust(45)
  host_data += "port: " + host_stats['machines'][host]['active_interface']['network_port']
  print host_data
    ...
    for redis_server in sorted(host_stats['machines'][host]['per_redis_server_interaction_time'].iterkeys()):
      ...
      if sort == "time":
        if host_stats['machines'][host]['per_redis_server_interaction_time'][redis_server].get(stat):
          host_data += stat + " seconds:" +  str(float('%.3f' % host_stats['machines'][host]['per_redis_server_interaction_time'][redis_server][stat]))
      elif sort == "speed":
        if host_stats['machines'][host]['per_redis_server_transfer_speed'][redis_server].get(stat):
          host_data += stat + " bytes/s:" +  str(int(host_stats['machines'][host]['per_redis_server_transfer_speed'][redis_server][stat]))
      print host_data
...
def print_netstat_data(host_stats, stat):
  ...
  for host in host_stats['machines'].iterkeys():
    if not host_stats['machines'][host]['active_interface']['netstat_summary'].get(stat):
      host_stats['machines'][host]['active_interface']['netstat_summary'][stat] = 0
    tmp[host] = float(host_stats['machines'][host]['active_interface']['netstat_summary'][stat])
...
def extract_range_clusters(host_stats):
  host_range_clusters = []
  for host in host_stats['machines'].iterkeys():
    host_range_clusters.append(host_stats['machines'][host]['range_clusters'])
...
def report_datacenter_statistics(data, stat, sort, percentile, host_stats, limit, switchcount, per_mps):
  ...
  shitty_switches = {}
  print
  print ("Data collected " +  str(datetime.timedelta(seconds=int(time.time() - host_stats['data_generation_time']))) + " minutes ago").center(120)
  print

  # For sysops-api insertion times, we take 50 samples per machine.  Otherwise for netstat data, the number of samples is number of hosts measured.
  description = ""
  ...
  ################################# Median ###########################################
  # Global median
  print ("The global median of " + description + " across " + str(len(data)) + "  hosts and " + total_measurements + " total measurements is " + str('%.3f' % np.median(data))).center(120)
  ...
  for percent in [1, 2, 3, 4, 5, 10, 50, 80, 90, 95, 96, 97, 98, 99]:
    print (str(percent) + "th percentile median is " + str('%.3f' % np.percentile(data, percent))).center(120)
  print
  ################################# Median ###########################################

  ################################# Mean #############################################
  # Global mean
  print ("The global mean of " + description + " across " + str(len(data)) + " hosts and " + total_measurements + " total measurements is " + str('%.3f' % np.mean(data))).center(120)
  ...
  for percent in [1, 2, 3, 4, 5, 10, 50, 80, 90, 95, 96, 97, 98, 99]:
    print (str(percent) + "th percentile mean is " + str('%.3f' % np.percentile(data, percent))).center(120)
  print
  ################################# Mean #############################################

  # If we supply --percentile 0 or a negative number, we only want to see the above summary report.  Exit after providing a client connected switch count.
  if percentile <= 0:
    for host in host_stats['machines'].iterkeys():
      if not shitty_switches.get(host_stats['machines'][host]['active_interface']['switch_name']):
        shitty_switches[host_stats['machines'][host]['active_interface']['switch_name']] = 1
      else:
        shitty_switches[host_stats['machines'][host]['active_interface']['switch_name']] += 1
    for swatch in sorted(shitty_switches, key=lambda x : shitty_switches[x]):
      print ("Switch: " + swatch.ljust(40) + "has " + str(shitty_switches[swatch]) + " clients connected as the active network interface.").center(120)
    sys.exit(0)

  # Discover which switches have hosts that have exceeded the discovered limit based on --stat and --percentile
  if limit:
    for host in host_stats['machines'].iterkeys():
      if stat == "mean" or stat == "median" or stat =="var":
    ...
    for swatch in sorted(shitty_switches, key=lambda x : shitty_switches[x]):
      if shitty_switches[swatch] >= switchcount:
          print ("Switch: " + swatch.ljust(40) + str(shitty_switches[swatch]) + " clients exceeded the threshold of " + str(percentile) + "th percentile of " + stat).center(120)
    print
#####################################################################################################################################
def reduce_working_set(host_stats, switch, hosts, mps):
  ...
  if mps:
    delete_hosts = []
    for host in host_stats['machines'].iterkeys():
      redis_servers = host_stats['machines'][host]['per_redis_server_interaction_time'].keys()
...
def return_machines_above_limit(host_stats, stat, limit, sort, range_connection):
  ...
  for host in host_stats['machines'].iterkeys():
    try:
      host_stats['machines'][host]['range_clusters'] = range_connection.expand("%index:" + host)
    except Exception:
...
def report_network_data_collection(host_stats, datacenter, stat, percentile, sort, report, per_mps, switchcount, netstat, mps, switch, hosts, range_connection, range_clusters):
  ...
  if not netstat:
    if stat == "median" or stat == "mean" or stat == "var":
      if sort == "time":
        limit = float('%.3f' % np.percentile(host_stats['total_redis_interaction_time'][stat], percentile))
        if report:
          report_datacenter_statistics(host_stats['total_redis_interaction_time'][stat], stat, sort, percentile, host_stats, limit, switchcount, per_mps)
      else:
        limit = float('%.3f' % np.percentile(host_stats['total_redis_transfer_speed'][stat], percentile))
        if report:
          report_datacenter_statistics(host_stats['total_redis_transfer_speed'][stat], stat, sort, percentile, host_stats, limit, switchcount, per_mps)
      # Reduce the working set, if provided
      if switch or hosts or mps:
        host_stats = reduce_working_set(host_stats, switch, hosts, mps)
      host_stats = return_machines_above_limit(host_stats, stat, limit, sort, range_connection)
      print ("The following hosts have exceeded the " + stat + " threshold of "  + str(float('%.3f' % limit)) + " at the " + str(percentile) + "th percentile").center(120)
      tmp = {}
      if sort == "time":
        for host in host_stats['machines'].iterkeys():
          tmp[host] = float(host_stats['machines'][host]['total_redis_interaction_time'][stat])
        for host in sorted(tmp, key=lambda x : tmp[x]):
          print_basic_host_info(host, host_stats, stat, sort, per_mps)
      elif sort == "speed":
        for host in host_stats['machines'].iterkeys():
          tmp[host] = float(host_stats['machines'][host]['total_redis_transfer_speed'][stat])
        if stat != "var":
        ...
        for host in host_stats['machines'].iterkeys():
            tmp[host] = host_stats['machines'][host]['active_interface']['switch_name']
        for host in sorted(tmp, key=lambda x : tmp[x]):
          print_basic_host_info(host, host_stats, stat, sort, per_mps)
      elif sort == "hostname":
  ...
  # netstat statistics
  if netstat:
    # Every iteration modifies host_stats to reduce the working set based off of the limit.  Reset host_stats on every cycle.
    ...
    for stat in netstat:
      ...
      for host in host_stats['machines'].iterkeys():
        if host_stats['machines'][host]['active_interface']['netstat_summary'].get(stat):
          netstat_data[stat].append(host_stats['machines'][host]['active_interface']['netstat_summary'][stat])
      netstat_data[stat] = map(float, netstat_data[stat])
      if len(netstat_data[stat]) > 0:
        limit = float('%.3f' % np.percentile(netstat_data[stat], percentile))
      else:
        continue
      if report:
        report_datacenter_statistics(netstat_data[stat], stat, sort, percentile, host_stats, limit, switchcount, per_mps)
      # Reduce the working set, if provided
      if switch or hosts or mps:
        host_stats = reduce_working_set(host_stats, switch, hosts, mps)
      host_stats = return_machines_above_limit(host_stats, stat, limit, sort, range_connection)
      print ("The following hosts have exceeded the " + stat + " threshold of "  + str(float('%.3f' % limit)) + " at the " + str(percentile) + "th percentile").center(120)
      # For netstat data, we process all hosts at once instead of processing them individually.
...
if __name__ == '__main__':
  ...
  host_stats = validate_json_data(host_stats, options.site, options.local_disk, workdir)
  ...
  # The map / lambda statements below attempt to deal with non-fqdn names passed on the CLI.  We at least need a prod/corp/stg/fin/etc suffix to append linkedin.com
  if options.filter_switch:
    options.filter_switch = options.filter_switch.split(',')
    options.filter_switch = map(lambda switch: re.sub("(prod$|stg$|corp$|fin$)", r"\g<1>.linkedin.com", switch), options.filter_switch)

  if options.filter_host:
    options.filter_host = options.filter_host.split(',')
    options.filter_host = map(lambda host: re.sub("(prod$|stg$|corp$|fin$)", r"\g<1>.linkedin.com", host), options.filter_host)

  if options.filter_mps:
    options.filter_mps = options.filter_mps.split(',')
    options.filter_mps = map(lambda mps: re.sub("(prod$|stg$|corp$|fin$)", r"\g<1>.linkedin.com", mps), options.filter_mps)
  ...
  if options.filter_range_query:
    options.filter_host = range_connection.expand(options.filter_range_query)
