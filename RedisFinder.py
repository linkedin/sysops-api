#!/usr/bin/python2.6
# filesource    \$HeadURL: svn+ssh://csvn@esv4-sysops-svn.corp.linkedin.com/export/content/sysops-svn/cfengine/branches/esv4-cfe-test.corp/generic_cf-agent_policies/config-general/manage_usr_local_admin/RedisFinder.py $
# version       \$Revision: 39818 $
# modifiedby    \$LastChangedBy: msvoboda $
# lastmodified  \$Date: 2013-04-01 15:19:13 +0000 (Mon, 01 Apr 2013) $
...
class RedisFinder():
  ...
  def query_range_for_redis_corelist(self):
    ...
    if self._scope == "local":
      # return the primary mps, which should already be load balanced via select_class generated by Cfengine.
      ...
      # slef._scope is set to something undefined.  set the corelist to None and break out. 
        ...
        for range_server in self._range_servers:
          try:
            ...
            if total_redis_corelist:
              ...
              # Since we've discovered our corelist, stop cycling through avilable range servers.
              break
            else:
              if self._verbose:
          ...
          if not total_redis_corelist:
            # if we get to this point, we were unable to populate the redis corelist.  Exit out.
            sys.exit(1)

        # Now, for each corelist, return a randomized redis server from that list and append to self._redis_corelist.
        for single_redis_core in total_redis_corelist:
          redis_servers_single_core = range_connection.expand("%" + single_redis_core)
          if self._verbose:
          ...
          if not self._return_randomized_servers:
            ...
            while redis_servers_single_core:
              ...
              random_redis_server_index = randrange(0,len(redis_servers_single_core))
              if self._verbose: 
                print "(+) RedisFinder.query_range_for_redis_corelist() random_redis_server_index is ", random_redis_server_index
              redis_server = redis_servers_single_core.pop(random_redis_server_index)
              try:
                redis_connection = redis.Redis(host=redis_server,port=6379,db=0,socket_timeout=1,charset='utf-8', errors='strict')       
                ...
                if redis_info:
                  ...
                  # Break out of here and move onto the next core to find the next redis server to push.
                  corelist_valid = True
                  break
